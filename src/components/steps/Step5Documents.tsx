...
const Step5Documents: React.FC<Props> = ({
  ...
  const handleResumeFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    onUploadedFilesChange({
      ...uploadedFiles,
      resumeFiles: [...uploadedFiles.resumeFiles, ...valid],
    });
    e.target.value = "";
  };

  const handleOtherDocFiles = (e: React.ChangeEvent<HTMLInputElement>) => {
    const selected = Array.from(e.target.files || []);
    const valid = selected.filter((f) => f.size <= MAX_FILE_MB * 1024 * 1024);
    onUploadedFilesChange({
      ...uploadedFiles,
      otherDocFiles: [...uploadedFiles.otherDocFiles, ...valid],
    });
    e.target.value = "";
  };
  ...
  const removeResumeFile = (index: number) => {
    onUploadedFilesChange({
      ...uploadedFiles,
      resumeFiles: uploadedFiles.resumeFiles.filter((_, i) => i !== index),
    });
  };

  const removeOtherDocFile = (index: number) => {
    onUploadedFilesChange({
      ...uploadedFiles,
      otherDocFiles: uploadedFiles.otherDocFiles.filter((_, i) => i !== index),
    });
  };
    ...
    <div className="step-section active">
      ...
      <div className="doc-link-card">
        <div className="doc-link-header">
          ...
          <div>
            ...
            <div className="doc-link-desc">
              Upload your resume or CV. Max {MAX_FILE_MB}MB per file. PDF
              preferred.
            </div>
          </div>
        </div>
        ...
        {uploadedFiles.resumeFiles.length > 0 && (
          <FileList
            files={uploadedFiles.resumeFiles}
            onRemove={removeResumeFile}
          />
        )}
      ...
      {/* ── Other Documents (certs, diploma, etc.) ──────────────────────── */}
      <div className="doc-link-card">
        <div className="doc-link-header">
          <span className="doc-link-icon">
            <FilePlus size={20} />
          ...
          <div>
            <div className="doc-link-title">Other Documents</div>
            <div className="doc-link-desc">
              Certificates, diploma, TOR, or any supporting documents. Max{" "}
        ...
        {uploadedFiles.otherDocFiles.length > 0 && (
          <FileList
            files={uploadedFiles.otherDocFiles}
            onRemove={removeOtherDocFile}
          />
        )}
